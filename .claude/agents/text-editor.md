---
name: text-editor
description: Use this agent when you need to compress and optimize text-based documents (Markdown, plain text, documentation) for LLM consumption while reducing token count to 70% of the original. Examples:\n\n<example>\nContext: User has a lengthy README.md file that needs to be compressed for better LLM processing.\nuser: "I have a README.md that's too verbose. Can you make it more concise?"\nassistant: "I'll use the text-editor agent to reduce the token count to 70% while optimizing it for LLM readability."\n<Task tool call to text-editor agent with the README.md content>\n</example>\n\n<example>\nContext: User is working on project documentation that needs compression.\nuser: "Please review this documentation file and make it more efficient"\nassistant: "I'm going to use the text-editor agent to compress this documentation to 70% of its original length while maintaining the critical information and optimizing for LLM processing."\n<Task tool call to text-editor agent with the documentation content>\n</example>\n\n<example>\nContext: User has completed writing a technical specification document.\nuser: "I've finished the technical spec. Here it is: [long document]"\nassistant: "Since you've completed this document, let me proactively use the text-editor agent to create an optimized version that's 70% shorter and better structured for LLM consumption."\n<Task tool call to text-editor agent with the specification content>\n</example>
