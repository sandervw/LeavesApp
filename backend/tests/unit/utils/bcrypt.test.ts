...
/**
 * These tests use the REAL bcrypt library (not mocked).
 *
 * Why? For beginners, it's easier to understand tests that use real implementations.
 * These are technically "integration tests" but they're placed in the unit folder
 * because they test isolated utility functions.
 *
 * Note: These tests are slower than mocked tests (bcrypt hashing is intentionally slow
 * for security), but they give you confidence the functions actually work.
 */
...
describe('bcrypt utils', () => {
  ...
  describe('compareValue', () => {
    ...
    it('should return false for invalid hash format', async () => {
      ...
      // Our compareValue catches errors and returns false
      expect(isMatch).toBe(false);
    });
